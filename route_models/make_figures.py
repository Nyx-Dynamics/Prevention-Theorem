...
# Load results
# Note: Renamed outputs as requested
# Current V0=1 results should be renamed low-inoculum systemic not mucosal
BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
RESULTS_DIR = os.path.join(BASE_DIR, 'SRC/multiscale_model/results')
OUTPUT_DIR = os.path.join(BASE_DIR, 'results/multiscale_model')

# Ensure output directory exists
os.makedirs(OUTPUT_DIR, exist_ok=True)

summary = pd.read_csv(os.path.join(RESULTS_DIR, 'mc_summary.csv'))
realizations = pd.read_csv(os.path.join(RESULTS_DIR, 'mc_realizations.csv'))
curves = pd.read_csv(os.path.join(RESULTS_DIR, 'mc_efficacy_curves.csv'))
...
for V0 in V0_grid:
    ...
    if col in curves.columns:
        label = (f'V₀ = 1 (low-inoculum systemic)' if V0 == 1
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'multiscale_results.png'),
            dpi=300, bbox_inches='tight', facecolor='white')
plt.savefig(os.path.join(OUTPUT_DIR, 'multiscale_results.pdf'),
            bbox_inches='tight', facecolor='white')
plt.close()

print(f"Saved: {os.path.relpath(OUTPUT_DIR, BASE_DIR)}/multiscale_results.png/.pdf")
...
comparisons = [
    ('Low-inoculum systemic (V₀=1)', '68-76h',         summary[summary['V0']==1]['t_crit_eta_05'].iloc[0]),
