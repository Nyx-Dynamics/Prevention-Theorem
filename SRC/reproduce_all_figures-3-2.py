...
def run_fig3_and_s3(csv_efficacy: str,
    ...
    # ── Fix 4: increase left margin so city name labels aren't clipped ────
    source = source.replace(
...
def run_panel_extraction():
    ...
    # ── Panel split coordinates (fractional — resolution-independent) ─────
    # These fractions were calibrated from the 2x2 subplot grid layout
    # and hold regardless of figure DPI or output size.
    PAR_X = int(round(par_w * 0.4922))   # ~50/50 horizontal split
    PAR_Y = int(round(par_h * 0.5189))   # ~52/48 vertical split (title adds height)
    STO_X = int(round(sto_w * 0.4942))
    STO_Y = int(round(sto_h * 0.5242))
    CIT_X = int(round(cit_w * 0.5091))   # city bar chart slightly wider
    CIT_Y = int(round(cit_h * 0.5397))
    FOC_X1 = int(round(foc_w * 0.3329))  # 1x3 panels: ~1/3 and ~2/3
    FOC_X2 = int(round(foc_w * 0.6894))

    print(f"  Computed split coords:")
