...
def resolve_data_dir(explicit_dir: Optional[str] = None) -> str:
    """
    Resolve the AIDSVu dataset directory across local and mounted environments.
    """
    if explicit_dir:
        return explicit_dir

    candidates = [
        PROJECT_ROOT / 'aidsvu datasets',
        PROJECT_ROOT / 'data',
        Path('/mnt/project'),
    ]
    ...
    return str(PROJECT_ROOT / 'aidsvu datasets')


def resolve_output_dir(explicit_dir: Optional[str] = None) -> str:
    """
    Resolve a writable output directory across local and mounted environments.
    """
    if explicit_dir:
        return explicit_dir
...
# Unsuppressed HIV VL: log-normal, grounded in Perelson 1996 within-host model
# Chronic untreated steady state: 10^3 to 10^6, median ~10^4.5
...
# These map to the pwid_barrier_analysis() scenarios in PEP_parenteral.py
...
def build_city_profiles(data_dir: str = None) -> pd.DataFrame:
    ...
    if not files:
        raise FileNotFoundError(
            f"No AIDSVu Excel files found in data directory: {data_dir}"
        )
...
def main():
    ...
    output_dir = os.path.join(resolve_output_dir(), 'city_analysis')
