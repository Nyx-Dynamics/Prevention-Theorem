...
\bibitem{tanner2025}
M.R. Tanner \textit{et al.}, Antiretroviral postexposure prophylaxis after sexual, injection drug use, or other nonoccupational exposure to HIV --- CDC recommendations, United States, 2025. \textit{MMWR Recomm. Rep.} \textbf{74}(1), 1--56 (2025). doi:\href{https://doi.org/10.15585/mmwr.rr7401a1}{10.15585/mmwr.rr7401a1}

\bibitem{wahl2023}
A. Wahl, L. Al-Harthi, HIV infection of non-classical cells in the brain. \textit{Retrovirology} \textbf{20}, 1 (2023). doi:\href{https://doi.org/10.1186/s12977-023-00616-9}{10.1186/s12977-023-00616-9}

\bibitem{siliciano2015}
J.D. Siliciano, R.F. Siliciano, Enhanced culture assay for detection and quantitation of latently infected, resting CD4+ T-cells. \textit{Methods Mol. Biol.} \textbf{1354}, 3--15 (2015).

\bibitem{perelson1997}
A.S. Perelson \textit{et al.}, Decay characteristics of HIV-1-infected compartments during combination therapy. \textit{Nature} \textbf{387}, 188--191 (1997). doi:\href{https://doi.org/10.1038/387188a0}{10.1038/387188a0}

\bibitem{perelson1996}
A.S. Perelson, A.U. Neumann, M. Markowitz, J.M. Leonard, D.D. Ho, HIV-1 dynamics \textit{in vivo}: virion clearance rate, infected cell life-span, and viral generation time. \textit{Science} \textbf{271}, 1582--1586 (1996). doi:\href{https://doi.org/10.1126/science.271.5255.1582}{10.1126/science.271.5255.1582}
...
\bibitem{baugher2025}
A.R. Baugher \textit{et al.}, Are we ending the HIV epidemic among persons who inject drugs? Key findings from 19 US cities. \textit{AIDS} \textbf{39}(12), 1813--1819 (2025). doi:\href{https://doi.org/10.1097/QAD.0000000000004249}{10.1097/QAD.0000000000004249}
...
A.E. Swanstrom \textit{et al.}, Lenacapavir protects macaques against intravenous SHIV challenge. \textit{Sci. Transl. Med.} (2023).

\bibitem{vidal2022}
S.J. Vidal \textit{et al.}, Long-acting antiretrovirals prevent SHIV infection in NHP mucosal challenge. \textit{J. Infect. Dis.} \textbf{226}, 2051--2059 (2022).
...
(\textbf{A}) Expected PEP efficacy $\Epep(t)$ as a function of hours post-exposure for mucosal (blue) and parenteral (red) routes, derived from the multiscale within-host model ($N = 500$ realizations per cell, CV $= 0.3$ on $\beta, c, \delta, \alpha$). Shaded bands: 95\% credible intervals from stochastic replications. Vertical shaded band at $\log_{10}(V\!L) < \log_{10}(200)$: \emph{U=U region (per PARTNER2 / modern operational definition; VL $<200$~c/mL); model not applicable, transmission risk $\approx 0$}. Dashed green vertical line: CDC 72-hour guideline. Dot-dash gray line: 5\% efficacy threshold ($\eta = 0.05$). Triangles: NHP protection data (open: complete protection; half-filled: partial; filled: no protection). Parenteral $\tcrit \approx 34.5$~h ($V_{0} = 10^{3}$, CV $= 0.3$); mucosal $\tcrit \approx 60.5$~h ($V_{0} = 1$, CV $= 0.3$); compression ratio $\approx 1.75\times$.
...
Values reproducible from \texttt{Nyx-Dynamics/Prevention-Theorem} at commit \texttt{[V2\_SUBMISSION\_SHA]} (\texttt{SRC/multiscale\_model/results\_v3/heterogeneity\_summary.csv}).}
\label{fig:route}
\end{figure}
...
\caption{\textbf{Stochastic efficacy analysis under source VL uncertainty.}
(\textbf{A}) Expected PEP efficacy $\pm$ 95\% CI for four source VL distributions (PWID untreated: mean $\log_{10} = 4.5$; PWID mixed treatment: 3.2; general community: 3.8; acute-infection-enriched: 5.0) over 0--120~h. Shaded regions: 95\% CI from Monte Carlo VL integration. Vertical shaded band at $\log_{10}(V\!L) < \log_{10}(200)$: U=U region (per PARTNER2 / modern operational definition); framework not applicable.
(\textbf{B}) Source VL probability densities for each distribution, illustrating the rightward shift and narrower spread of the acute-enriched network.
(\textbf{C}) Temporal regime classification: CI width as a function of time. Regime~1 (white): 0--22~h, narrow CI, VL knowledge low value. Regime~2 (gray): 22--78~h (PWID/acute) or 22--88~h (mixed/general), maximal CI (peak 39.7~pp at 49~h), VL knowledge clinically actionable. Regime~3 (dark): $>$78~h (PWID/acute) or $>$88~h (mixed/general), CI collapses, outcome uniformly poor.
(\textbf{D}) VL knowledge premium (absolute difference, known vs.\ unknown source VL), peaking at 2.94~pp at 21~h (PWID untreated) and 3.76~pp (acute-enriched); non-monotonic plateau 18--36~h preserved as real kinetic signal.}
...
(\textbf{D}) Actual vs.\ 24-hour counterfactual efficacy by city. For 33/34 cities, actual efficacy exceeds counterfactual because structural delays are currently shorter than 24~h; Hartford is the exception. The 19.5~pp range across cities is almost entirely attributable to structural delay ($r = -0.935$) rather than community VL ($r = -0.084$). \textit{Note:} city-level AIDSVu late-diagnosis and viral suppression data are aggregated across mode of HIV acquisition and cannot be reliably subset to U=U status; this figure pools all source-VL strata in the surveillance denominator, consistent with the Methods specification of $\Faccess$.}
\label{fig:city}
\end{figure}
