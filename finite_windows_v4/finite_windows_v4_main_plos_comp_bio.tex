...
\subsection*{Pharmacy-access barrier: envelope-corridor analysis}

The structural delay formula (Eq.~\ref{eq:delay}) captures system-level friction up to clinical decision-making but treats prescription writing and first-dose ingestion as effectively coincident. In real-world ED practice this assumption is often false. The CDC 2025 nPEP recommendations explicitly document that pharmacy dispensing practices have sometimes been a barrier to timely nPEP access\cite{tanner2025}; a recent quality-improvement initiative at Alameda Health System reported that initial PEP compliance is materially impeded by limited access to pharmacies and cost\cite{white2025nPEP}; national survey data from 587 U.S. EDs show that only 65.3\% of EDs offer both HIV testing and PEP to sexual assault survivors\cite{lopezcastillo2020,white2025nPEP}.

We treat the pharmacy-access barrier $\Delta t_{\text{pharmacy}} \in \{0, 2, 4, 6, 8, 10, 12\}$~hours as an upstream gating event: patients who fail to acquire medication before $\tcrit$ never accumulate drug, and PK-driven efficacy is defined only conditional on successful acquisition. Under this causal structure, the achievable $\Epep$ envelope is a two-dimensional corridor in $(t_{\rm acq}, \Epep)$ space, bounded above by perfect-adherence kinetics ($\rho = 1.0$, cliff at $\tcrit(\rho{=}1.0) = 34.0$~h) and below by low-adherence kinetics ($\rho = 0.30$, cliff at $32.0$~h). Pharmacy delay does not compress this corridor; it slides each city rightward along $t_{\rm acq} = \Delta t_{\text{structural}} + \Delta t_{\text{pharmacy}}$ toward the cliff (Section~S14).

The effect is concentrated in already-high-barrier cities. Hartford (structural delay 24.4~h) enters at $t_{\rm acq} = 24.4$~h with $\Epep^{\rm upper} = 0.479$ and $\Epep^{\rm lower} = 0.098$ --- the corridor is already narrow at this position. A pharmacy delay of $\Delta t_{\text{pharmacy}} = 8$~h slides Hartford to $t_{\rm acq} = 32.4$~h with $\Epep^{\rm upper} = 0.175$, still on the corridor. At $\Delta t_{\text{pharmacy}} = 10$~h, Hartford reaches $t_{\rm acq} = 34.4$~h, exceeding $\tcrit(\rho{=}1.0) = 34.0$~h, and is displaced \emph{off} the corridor; $\Epep$ collapses to zero --- not as a probabilistic floor effect, but because the drug never accumulates before viral integration is complete in that regime. The other five high-vulnerability cities in Fig.~S1 (San Juan, Jackson, Phoenix, Denver, Milwaukee) remain on the corridor with $\Epep^{\rm upper} > 0.96$ even at $\Delta t_{\text{pharmacy}} = 12$~h because their baseline structural delays are 1.3--8.1~h, leaving substantial margin to $\tcrit(\rho{=}1.0)$. Across the 34-city panel, the cohort mean of $\Epep^{\rm upper}$ declines from $0.962$ at $\Delta t_{\text{pharmacy}} = 0$ to $0.865$ at $\Delta t_{\text{pharmacy}} = 12$~h. The substantively interpretable summary is the off-corridor count: at $\Delta t_{\text{pharmacy}} \in \{0, 2, 4, 6, 8\}$~h, zero of 34 cities are displaced past $\tcrit$; at $\Delta t_{\text{pharmacy}} \in \{10, 12\}$~h, one city (Hartford) is displaced. The analytical $\Faccess \times \tcrit$ bound (Eq.~\ref{eq:bound}) therefore remains a separate population-level consistency check; the pharmacy analysis answers a different mechanistic question: which cities are displaced along the PK/PD--integration corridor toward or beyond the parenteral cliff.
...
The pharmacy-corridor analysis and the $\Faccess \times \tcrit$ inequality answer complementary questions. The corridor analysis is individual- and city-position specific: it asks whether acquisition time remains inside the route-specific PK/PD--integration window. The $\Faccess \times \tcrit$ inequality is population-distributional: it asks how large expected prevention could be under a conservative upper-limit timing assumption. We therefore use the corridor for mechanistic interpretation of access barriers and the $\Faccess$ bound only as an analytical consistency check on the scale of population-level failure.

\section*{Population-level implications: cascade estimates, outbreak forecasts, and analytical consistency bound}
...
\subsection*{(c) Analytical $\Faccess \times \tcrit$ consistency bound}
...
\label{eq:bound}
\end{equation}
(Proposition~S6.1). The bound requires only the fraction of the population accessing PEP within the parenteral window --- not the full parametric form of $\Faccess$. It should be interpreted as a conservative analytical ceiling, not as a full population preventability estimator, because it collapses access timing into a scalar distributional term and does not model the per-patient PK/PD corridor described above. Modeling access delay as log-normal (median 96~h, geometric SD 2.0; modeling choice informed by qualitative discussion in the PWID PEP-access literature\cite{taylor2019,baugher2025}; the specific median is a defensible parameterization rather than an empirical anchor lifted from primary timing data) yields $\Faccess(34.5~\text{h}) \approx 0.070$. With $\eta = 0.05$ and $\emax = 0.95$, Equation~\ref{eq:bound} yields $\Ebarpep \leq 0.070 \times 0.95 + 0.930 \times 0.05 \approx 11.3\%$. Sensitivity analysis across plausible median values $F_{\text{median}} \in \{72, 96, 110, 120\}$~hours yields analytical ceiling estimates of 8.2\%--18.0\% (Table~\ref{tab:bound}); in all cases, the ceiling remains one to four orders of magnitude above the cascade point estimates in (a), supporting the conclusion that the cascade estimates are not artifacts of an overly pessimistic access assumption.

\subsection*{Convergent finding}

These analyses converge at different inferential levels. The cascade model supplies central estimates of realized prevention success; the outbreak model supplies an epidemiologic consequence of those barriers; the PK/PD corridor identifies the mechanism by which structural and pharmacy delays move acquisition time toward the parenteral integration cliff; and the $\Faccess \times \tcrit$ inequality provides a conservative upper-limit check. Together, they support the conclusion that PEP cannot serve as a population-level safety net for PWID under current structural conditions. The analytical bound is deliberately least pessimistic; the cascade-based central estimates are 100--3{,}000$\times$ lower and remain qualitatively consistent.
...
PEP is not the safety net for parenteral HIV exposure that it is for sexual transmission. We prove that proviral integration establishes a finite, route-specific prevention window; that this window is approximately 1.75-fold shorter for parenteral than for mucosal exposure; and that structural access delays for PWID systematically consume this window. Across the cascade model, system-level outbreak forecasting, PK/PD corridor analysis, and analytical $\Faccess \times \tcrit$ consistency bound, population-level PEP efficacy for PWID under current structural conditions is estimated at approximately 0.003--0.7\% in cascade models and remains below an intentionally conservative analytical ceiling of approximately 11\%, with corresponding 92.7\% 10-year outbreak probability under current policy. The conclusion is not a statement against PEP for individuals who present in time. It is a quantitative argument that pre-exposure prevention is the primary biomedical tool available for this population --- and that current deployment levels are insufficient to close the gap.
...
Zenodo (v4.0.0 DOI: \href{https://doi.org/10.5281/zenodo.20559022}{10.5281/zenodo.20559022}).
...
\textbf{Supplementary Materials:} Supplementary Text S1--S14 with complete proofs, parameter tables, extended methods, NHP concordance details, PK/PD implementation, pharmacy-access corridor analysis, and figure inventory.
...
\begin{figure}[!ht]
\centering
\includegraphics[width=\textwidth]{Figure_1_VL_Compression_PEP_Parenteral.png}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=\textwidth]{Figure_2_Stochastic_Efficacy_VL_Uncertainty.png}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=\textwidth]{Figure_3_City_Stratified_PEP_Efficacy.png}
...
\begin{table}[!ht]
\centering
\caption{\textbf{Analytical $\Faccess \times \tcrit$ consistency-bound sensitivity} (Equation~\ref{eq:bound}, $\eta = 0.05$, $\emax = 0.95$, $\tcrit^{(p)} = 34.5$~h).}
\label{tab:bound}
\small
\setlength{\tabcolsep}{4pt}
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{$\Faccess$ parameterization} & \textbf{$\Faccess(34.5\,\text{h})$} & \textbf{Analytical ceiling for $\Ebarpep$} \\\midrule
...
\vspace{0.3em}
\footnotesize
\textbf{Reading the bound.} The $\Faccess \times \tcrit$ inequality is a conservative analytical ceiling, not a full population preventability estimator. Cascade-based point estimates (Section: Population-level implications) yield 0.003--0.7\% under current US policy, one to four orders of magnitude below this ceiling. The qualitative conclusion is supported primarily by the cascade estimates and the PK/PD corridor analysis; the $\Faccess$ value functions as a consistency check and depends on the modeled access-delay median.
\end{table}
