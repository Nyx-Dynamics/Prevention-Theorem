...
def build_city_profiles(data_dir: str = '/mnt/project') -> pd.DataFrame:
    """
    ...
    for fp in files:
        ...
        row = {**profile, **vl_params, **delay_params,
               'estimated_annual_pwid_exposures': round(estimated_pwid_exposures) if pd.notna(estimated_pwid_exposures) else 0}
        rows.append(row)
...
def main():
    ...
    # --- Parse all cities ---
    city_df = build_city_profiles(data_dir='/mnt/project')
