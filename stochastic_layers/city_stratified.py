...
THEORETICAL BRIDGE TO PEP MODEL:
    ...
    1. VL distribution parameters (mean_log10, sd_log10):
       ...
       Unsuppressed VL distribution ~ LogNormal(4.5, 1.1) [NHBS/NHAS surveillance]
...
# SRC_DIR = PROJECT_ROOT / 'SRC'  # Old SRC dir is now split
sys.path.append(str(PROJECT_ROOT))
sys.path.append(str(PROJECT_ROOT / 'route_models'))
sys.path.append(str(PROJECT_ROOT / 'core_theorem'))
...
def run_city_stratified_analysis(city_df: pd.DataFrame,
    ...
    for _, city in city_df.iterrows():
        ...
        results.append({
            ...
            'p_futile_pct': round(np.mean(efficacies < 0.05) * 100, 2),
        ...
        print(f"  {city_name:<18} | delay={delay_h:.0f}h | "
              ...
              f"P(futile)={np.mean(efficacies < 0.05) * 100:.1f}%")
...
def main():
    ...
    print(f"Prevention Theorem — City-Stratified Stochastic PEP Model | {TIMESTAMP}")
    print("=" * 70)
    ...
    city_df.to_csv(path1, index=False)
    print(f"\n[{TIMESTAMP}] Saved master city profiles: {path1} ({len(city_df)} cities)")
    ...
    print(city_df['delay_category'].value_counts().to_string())
    print("\nVL profile distribution:")
    print(city_df['vl_profile'].value_counts().to_string())

    # Top 5 highest burden cities
    print("\nTop 5 cities by structural delay (worst barriers):")
    worst = city_df.nlargest(5, 'estimated_structural_delay_h')[
    ...
    highest_vl = city_df.nlargest(5, 'mean_log10')[
    ...
    pep_results.to_csv(path2, index=False)
    print(f"\n[{TIMESTAMP}] Saved city PEP results: {path2}")
    ...
    pep_24h.to_csv(path3, index=False)
    print(f"[{TIMESTAMP}] Saved counterfactual results: {path3}")
    ...
    diff = merged['pep_24h_counterfactual_pct'] - merged['pep_mean_efficacy_pct']
    merged['efficacy_loss_due_to_delay_pp'] = diff.clip(lower=0).round(2)
    merged['efficacy_gain_due_to_early_pp'] = (-diff).clip(lower=0).round(2)

    merged = merged.sort_values('efficacy_loss_due_to_delay_pp',
                                 ascending=False)

    path4 = os.path.join(output_dir, 'city_structural_delay_cost.csv')
    merged.to_csv(path4, index=False)
    print(f"[{TIMESTAMP}] Saved structural delay cost: {path4}")
