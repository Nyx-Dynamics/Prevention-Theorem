...
#       which is appropriate for the kinetic parameters below but not the
#       community distribution reference. Cite NHBS for this value.
...
# MIN_SEEDING_MIDPOINT: virion t½ = 0.24 days = ~6h (Table 1)
#   Below ~6h post-exposure, free virions haven't even cleared; seeding
...
MIN_SEEDING_MIDPOINT = 6.0       # hours — anchored to Perelson virion t½ (~6h)
...
class ParenteralExposureModel:
    ...
    def p_integration_complete(self, hours_post_exposure: float) -> float:
        ...
        k = 0.20  # Steeper still: integration transition is sharp
    ...
    def pep_efficacy(self,
        ...
        efficacy_if_seeded_not_integrated = 0.40 * adherence  # Lower than mucosal
        # (parenteral: faster viral replication, less time to clear)
...
def plot_vl_sweep(save_path: str = None):
    ...
    fig.suptitle(
        'Viral Load as Window Compressor\n'
        'PEP Parenteral Extension',
        fontsize=16, fontweight='bold', x=0.05, ha='left', y=0.96
    )
    ...
    # Add extra padding for panel extraction
...
def pwid_barrier_analysis():
    ...
    print("\nKey finding: At VL >100,000, even a 18h structural delay")
    print("(typical criminalization scenario) reduces PEP efficacy to <30%.")
...
def main():
    ...
    import matplotlib
    matplotlib.use('Agg')
    plot_vl_sweep(save_path='FIgure_1_pep_parenteral_vl_sweep.png')
