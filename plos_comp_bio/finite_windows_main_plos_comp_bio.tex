...
\begin{figure}[!ht]
\centering
% \includegraphics[width=\textwidth]{science_template/Figure_1_VL_Compression_PEP_Parenteral.png} % PLOS submission: upload figures separately
\caption{\textbf{Source viral load compresses the parenteral PEP window.}
(\textbf{A}) Expected PEP efficacy $\Epep(t)$ as a function of hours from exposure to PEP initiation across eight source viral load values from 50~c/mL (suppressed; dark green) to $10^{6}$~c/mL (acute viremia; dark red), parenteral route ($V_{0}$ scales with source VL through the founder-population mapping; see Methods). Reference lines mark the 24-hour optimal-initiation threshold (blue dashed), the CDC 72-hour guideline (gray dashed), and the 50\% efficacy threshold (gray dot-dash). The leftward shift of the efficacy curve with increasing source VL is the visual signature of window compression formalized in panel B.
(\textbf{B}) Compression of the effective PEP window as a function of source VL. The effective PEP window (red, solid) tracks $\tcrit(\eta = 0.05)$ at each VL; the seeding midpoint (blue dashed) tracks median $\Tseed$; the integration-complete time (orange dashed) tracks median $\Tint$ beyond which $\Epep \to \emin = 0$. The pink shaded region is the time interval within which any PEP intervention must be initiated to retain $\Epep > \eta$. For source $\text{VL} \gtrsim 5 \times 10^{4}$~c/mL, the effective window has compressed below the CDC 72-hour guideline; for source $\text{VL} \gtrsim 10^{6}$~c/mL, it has compressed to within hours of the 24-hour threshold.
(\textbf{C}) Probability that proviral integration is already complete --- and therefore that PEP can no longer prevent infection --- as a function of source VL at three clinically relevant timepoints: 24~h (green), 48~h (orange), and 72~h (red). Horizontal references mark the 50\% threshold and the operational definition of futility ($P(\text{integrated}) \geq 90\%$, dashed dark red). At the 72-hour CDC guideline, integration is essentially complete for all source $\text{VL} \gtrsim 10^{4}$~c/mL.
(\textbf{D}) Direct comparison of mucosal (black, solid) vs.\ parenteral (dashed: VL 200, 10{,}000, 100{,}000 c/mL) PEP efficacy decay. Mucosal efficacy is shown as a single curve because the post-epithelial-attenuation framework reduces effective inoculum to $V_{0} = 1$ regardless of source VL; parenteral efficacy is stratified to show the joint effect of route (direct intravascular delivery) and source viremia (scaling of the founder inoculum). U=U scoping (framework applies only to transmission-competent exposures with source $\text{VL} \geq 200$~c/mL per PARTNER2; see Methods) is implicit throughout.
Values reproducible from \texttt{Nyx-Dynamics/Prevention-Theorem} at commit \texttt{37e27ea} via \texttt{SRC/perelson/parenteral/PEP\_parenteral.py} (\texttt{plot\_vl\_sweep}).}
\label{fig:route}
\end{figure}

\begin{figure}[!ht]
\centering
% \includegraphics[width=\textwidth]{science_template/Figure_2_Stochastic_Efficacy_VL_Uncertainty.png} % PLOS submission: upload figures separately
...
\begin{figure}[!ht]
\centering
% \includegraphics[width=\textwidth]{science_template/Figure_3_City_Stratified_PEP_Efficacy.png} % PLOS submission: upload figures separately
