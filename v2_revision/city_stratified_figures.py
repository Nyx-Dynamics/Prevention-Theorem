...
RESULTS_DIR = PROJECT_ROOT / 'results' / 'city_analysis'

# Ensure results directory exists
RESULTS_DIR.mkdir(parents=True, exist_ok=True)

# ── Load data ──────────────────────────────────────────────────────────────
# Load from RESULTS_DIR where AIDSVu_city_stratified_perelson.py saves them
...
legend_patches = [
    ...
    mpatches.Patch(color=DELAY_COLORS['moderate_barrier'], label='Moderate barrier (8–18h)'),
    mpatches.Patch(color=DELAY_COLORS['high_barrier'],     label='High barrier (18–30h)'),
]
...
# Half-life annotation — matches v2 multiscale model parameterization (c=23/day,
# t½ ≈ 0.7 h; Ramratnam 1999 / Perelson 1997). The earlier Perelson 1996 estimate
# (c≈3, t½≈6h) is superseded by Ramratnam's apheresis measurements.
ax.annotate('Virion half-life ~0.7h\n(c=23 day$^{-1}$, Ramratnam 1999)',
...
City-specific structural delay across 34 high-burden metropolitan areas measured expected PEP efficacy. Milwaukee WI demonstrated lowest barrier delay (1.9 hr). Seven cities exhibited moderate barriers
(structural delay 8–18h): San Juan PR, Palm Beach County FL, Columbia SC, New Haven CT, Charleston SC, Raleigh NC, and Houston TX. Hartford, CT (24.4 hr delay) was the sole city in the high barrier category.
...
print(f"[{TIMESTAMP}] Saved: city_analysis_manuscript_text.txt")
