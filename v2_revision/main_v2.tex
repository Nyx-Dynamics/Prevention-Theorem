...
(\textbf{A}) Expected PEP efficacy $\pm$ 95\% CI for four source VL distributions (PWID untreated: mean $\log_{10} = 4.5$; PWID mixed treatment: 3.2; general community: 3.8; acute-infection-enriched: 5.0) over 0--120~h. Shaded regions: 95\% CI from Monte Carlo VL integration. The framework applies to transmission-competent exposures only (source VL $\geq 200$ c/mL); U=U exposures are outside scope (Methods).
(\textbf{B}) Source VL probability densities for each distribution, illustrating the rightward shift and narrower spread of the acute-enriched network. The U=U threshold (source VL $<200$ c/mL; per PARTNER2 / modern operational definition) lies to the left of all four plotted distributions; the framework is silent in that region.
