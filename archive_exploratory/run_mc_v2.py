...
def run_mc_at_V0(
    ...
    # V0=1 is typically mucosal, higher V0 (e.g., 1000) is parenteral
    ...
    for k in range(n_realizations):
        ...
        rows.append({
            ...
            'T_int_hours': T_int_total,  # Backward compatibility
        })
...
def analyze_route(
    ...
    return {
        ...
        'n_total': n_total,
        'n_integrated': n_int,
        'T_int_median': float(np.median(integrated['T_int_total'])),
        'T_int_mean':   float(np.mean(integrated['T_int_total'])),
        'T_int_p5':     float(np.percentile(integrated['T_int_total'], 5)),
        'T_int_p95':    float(np.percentile(integrated['T_int_total'], 95)),
        'T_seed_median': float(np.median(integrated['T_seed_total'])),
    }
