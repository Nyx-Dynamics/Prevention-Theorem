...
print(f"[{TIMESTAMP}] Generating stochastic_summary_timepoints...")
rows = []
...
df_summary.to_csv('pep_stochastic_summary_timepoints_corrected.csv', index=False)
print(f"  [{TIMESTAMP}] Done. {len(df_summary)} rows saved to pep_stochastic_summary_timepoints_corrected.csv")
...
df_curves.to_csv('pep_stochastic_efficacy_curves_corrected.csv', index=False)
print(f"  [{TIMESTAMP}] Done. {len(df_curves)} rows saved to pep_stochastic_efficacy_curves_corrected.csv")
...
df_regimes.to_csv('pep_stochastic_ci_width_regimes_corrected.csv', index=False)
print(f"  [{TIMESTAMP}] Done. {len(df_regimes)} rows saved to pep_stochastic_ci_width_regimes_corrected.csv")
...
df_premium.to_csv('pep_vl_knowledge_premium_corrected.csv', index=False)
print(f"  [{TIMESTAMP}] Done. {len(df_premium)} rows saved to pep_vl_knowledge_premium_corrected.csv")
...
df_city.to_csv('city_pep_efficacy_results_corrected.csv', index=False)
print(f"  [{TIMESTAMP}] City efficacy: {len(df_city)} cities saved to city_pep_efficacy_results_corrected.csv")
...
df_city_cf = df_city.merge(df_cf[['city','pep_mean_efficacy_pct_24h_counterfactual',
                           ...
                           on='city')
df_city_cf.to_csv('city_pep_efficacy_24h_counterfactual_corrected.csv', index=False)
print(f"  [{TIMESTAMP}] Counterfactual: {len(df_city_cf)} cities saved to city_pep_efficacy_24h_counterfactual_corrected.csv")
...
df_cost.to_csv('city_structural_delay_cost_corrected.csv', index=False)
print(f"  [{TIMESTAMP}] Delay cost: {len(df_cost)} cities saved to city_structural_delay_cost_corrected.csv")
...
print(f"\n[{TIMESTAMP}] All CSVs written to current directory")
